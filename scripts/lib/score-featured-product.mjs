/** Mirror of src/utils/scoreFeaturedProduct.ts for Node publish scripts. */
...
export function scoreProduct(product) {
  ...
  score += (product.pros?.length || 0) * 5;
  score += Math.min(product.specs?.length || 0, 6) * 3;
...
export function pickBestProduct(products) {
...
export function bestPickBadge(product) {
  return product.price <= 500 ? "Best pick" : "Best buy pick";
}

export function reorderFeaturedProduct(products) {
  ...
  const winnerCopy = { ...winner, badge: winnerBadge };
